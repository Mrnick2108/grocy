...
class StockApiController extends BaseApiController
{
	...
	public function BarcodeSelected(Request $request, Response $response)
	{
		User::checkPermission($request, User::PERMISSION_STOCK_EDIT);
		...
		try {
			...
			if (!array_key_exists('selectedProduct', $requestBody)) {
				throw new \Exception('A selected_product is required');
			}

			$addFoundProduct = false;
			if (array_key_exists('add', $requestBody) && ($requestBody['add'] === 'true' || $requestBody['add'] === 1)) {
				$addFoundProduct = true;
			}

			return $this->ApiResponse($response, $this->getStockService()->ExternalBarcodeSelectedSave($requestBody['selectedProduct'], $addFoundProduct));
		} catch (\Exception $ex) {
			return $this->GenericErrorResponse($response, $ex->getMessage());
		}
	}

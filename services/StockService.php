...
class StockService extends BaseService
{
	...
	public function SaveExternalBarcodeOutput($pluginOutput, $addFoundProduct)
	{
			...
			if ($addFoundProduct === true) {
				...
				if (isset($pluginOutput['__image_url']) && !empty($pluginOutput['__image_url'])) {
					try {
						...
						$extension = pathinfo(parse_url($pluginOutput['__image_url'], PHP_URL_PATH), PATHINFO_EXTENSION);

						if ($extension == '') {
							// Fallback if no extension can be found in URL
						...
						$fileName = $pluginOutput['__barcode'] . '.' . $extension;
	...
	public function ExternalBarcodeLookup($barcode, $addFoundProduct)
	{
		...
		//If PluginOutput is array returnn the array
		if ($pluginOutput !== null) {
		...
		return $this->SaveExternalBarcodeOutput($pluginOutput, $addFoundProduct);
	}

	public function ExternalBarcodeSelectedSave($selectedProduct, $addFoundProduct)
	{
		$plugin = $this->LoadExternalBarcodeLookupPlugin();

		$pluginOutput = $plugin->SaveBarcode($selectedProduct);
